...
// CURRENT WINDOW
var folder_editor_win = remote.getCurrentWindow();
...
function getFolder(){
  ...
  if (result) {
    generateTreePanel(result.slice(0, result.length - 1)); 
    folder_editor_win.maximize();
    folder_editor_win.show()
  } else {
    folder_editor_win.close();
  }
}
getFolder();
...
function generateTreePanel(folder_path){
	...
	if(tree){
		explorer_panel.innerHTML += '<div class="has-background-warning has-text-dark" style="margin-bottom: 8px">' +
	'		' + folder_name +
	'	</div>' +
	...
	'	</ul>'
		...
		explorer_panel.innerHTML += '<h1 style="color: red;">Path not found -> ' + folder_path + '<h1>'
	}
}
